...
/*
Classification( 분류 ) : 입력 데이터를 다양한 클래스 중 하나로 분류하는 작업을 말한다. 
					=> 이는 주어진 데이터를 사전 정의된 카테고리나 클래스로 할당하는 과정을 의미하며, 
					=> 머신러닝 및 딥러닝에서 매우 중요한 작업 중 하나다.

분류 작업의 핵심 요소
	입력 데이터 : 분류 작업의 시작점은 입력 데이터 
				=> 이 데이터는 이미지, 텍스트, 음성 등 다양한 형태일 수 있으며, 
				=> 각각의 데이터 포인트는 하나 이상의 특성(feature)으로 구성된다. 
				=> 예를 들어, 이미지 분류에서는 각 픽셀의 값이 특성이 될 수 있다.
	클래스 레이블 : 분류 작업은 데이터 포인트를 어떤 클래스 또는 범주에 할당하는 것을 목표로 한다. 
				=> 클래스는 개별 객체 또는 패턴을 나타내며, 이러한 클래스는 사전에 정의되어야 한다. 
				=> 이진 분류( binary classification )의 경우에는 두 개의 클래스로 나뉠 수 있고, 
				=> 다중 클래스 분류( multi-class classification )의 경우에는 두 개 이상의 클래스가 있을 수 있다.
	모델 학습 : 분류 작업은 모델을 학습시켜 입력 데이터와 클래스 레이블 간의 관계를 학습하는 것으로 시작한다. 
			=> 이 학습 과정에서 모델은 데이터에서 패턴을 인식하고, 이러한 패턴을 기반으로 예측을 수행할 수 있는 특성을 학습한다.
	예측 : 학습된 모델을 사용하여 새로운, 이전에 보지 못한 데이터에 대한 클래스 레이블을 예측한다. 
		=> 모델은 입력 데이터를 분석하고 예측을 수행한 후 해당 데이터가 어떤 클래스에 속하는지 결정한다.

분류 작업은 다양한 응용 분야에서 사용된다.
	이미지 분류 : 객체 인식, 얼굴 인식, 병의 진단 등 이미지 기반의 다양한 작업에 사용된다.
	텍스트 분류 : 스팸 메일 필터링, 감성 분석( sentiment analysis ), 주제 분류 등 텍스트 데이터를 처리하는 데 사용된다.
	음성 분류 : 음성 인식, 화자 인식, 음성 감정 분석 등 음성 데이터를 다루는 작업에 활용된다.
	자율 주행 분야 : 도로에서 다른 차량 및 보행자를 분류하여 안전 운전에 사용된다.

딥러닝에서는 신경망 모델, 특히 컨볼루션 신경망( CNN )과 순환 신경망( RNN )과 같은 복잡한 구조를 
사용하여 분류 작업을 수행하는 데 많이 활용된다. 
이러한 모델은 데이터의 특징을 자동으로 추출하고 다양한 분류 작업에서 우수한 성능을 보이며, 
대규모 데이터셋에서 효과적으로 학습된다.
*/

/* ----- < Classifier loss 학습 > ----- */

/*
Classifier Loss( 분류기 손실 )학습 : 분류 모델을 학습시키는 데 사용되는 손실 함수( loss function )를 말한다. 
이 손실 함수는 모델의 출력과 실제 클래스 레이블 사이의 차이를 측정하며, 이 차이를 최소화하도록 모델의 가중치( weight )와 편향( bias )을 조정하는데 사용된다. 
Classifier Loss 학습은 분류 문제를 다룰 때 주로 사용되며, 다양한 분류 작업에서 모델을 효과적으로 학습시키는 데 중요

Classifier Loss 학습의 핵심 요소
	분류 모델 : Classifier Loss 학습의 시작점은 분류 모델이다. 
			=> 이 모델은 입력 데이터를 받아와 여러 클래스 중 하나에 해당하는 출력을 생성한다. 
			=> 분류 모델은 일반적으로 신경망, 특히 딥러닝 모델인 컨볼루션 신경망( CNN ) 또는 순환 신경망( RNN ) 등을 사용하여 구성된다.
	클래스 레이블 : Classifier Loss 학습에서는 각 데이터 포인트에 대해 실제 클래스 레이블이 필요
				=> 이 클래스 레이블은 데이터가 어떤 클래스에 속하는지 나타낸다. 
				=> 이진 분류의 경우에는 두 가지 클래스 중 하나를 선택하고, 다중 클래스 분류의 경우에는 여러 클래스 중 하나를 선택한다.
	손실 함수 ( Loss Function ) : 분류 모델의 출력과 실제 클래스 레이블 간의 차이를 측정하는 함수가 손실 함수다. 
								=> 가장 일반적인 손실 함수로는 교차 엔트로피 손실( Cross-Entropy Loss )이 사용된다. 
								=> 이 손실 함수는 모델이 예측한 클래스 확률 분포와 실제 클래스 레이블 간의 차이를 측정하여 모델을 학습시킨다.
	학습 과정 : Classifier Loss 학습은 학습 데이터를 사용하여 손실 함수를 최소화하도록 모델의 가중치와 편향을 조정하는 것으로 진행된다. 
			=> 이 과정에서 역전파( Backpropagation ) 알고리즘을 사용하여 손실 함수의 그래디언트( 기울기 )를 계산하고, 
			=> 경사 하강법( Gradient Descent ) 또는 그 변형을 사용하여 모델을 업데이트한다.
	예측 : 학습된 모델을 사용하여 새로운 데이터에 대한 예측을 수행할 수 있다. 
		=> 모델은 입력 데이터를 분석하고 가장 높은 확률을 갖는 클래스를 선택하여 해당 데이터의 클래스 레이블을 예측한다.

Classifier Loss 학습은 다양한 분류 작업에 적용된다. 
이를 통해 이미지 분류, 텍스트 분류, 음성 인식, 객체 검출 등 다양한 응용 분야에서 모델을 학습시키고 예측을 수행할 수 있다. 
손실 함수의 선택과 모델의 구성은 주어진 문제와 데이터에 따라 다르며, 최종 목표는 모델이 정확하게 분류를 수행하도록 하는 것이다.
*/