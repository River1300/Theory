...
/*
디지털로 색상을 표현하다 보니 현실 세계의 빛을 표현하는 개념을 지정할 필요가 있었다.

RGB 색 공간
	삼원색을 혼합하면 명도가 올라가는( 밝아지는 ) 방식으로 색상을 표현한다.
CMYK 색 공간
	인쇄에서 사용되는 잉크 옥색, 자홍색, 노랑, 검정을 혼합하는 방식이다.
HSV 색 공간
	예술 분야에서 사용하는 방식으로 색상, 채도, 명도로 표현한다.
YCbCr
	Y 는 휘도, Cb Cr은 색상의 차이로 원본 RGB값을 줄여서 표현하는 방식이다.
	MPEG, JPEG같은 압축 이미지에서 주로 사용한다.
*/

/* RGB 색 공간 */

/*
색의 3요소인 RGB로 효율적으로 표시하다 보니 RGB로 표현 가능한 3차원 공간을 고안해 내었다.
즉, X, Y, Z축을 각각 RGB 색상으로 표현한 이런 형태를 색상 공간이라 부른다.

과거 PC 게임 환경에서는 크게 중요하지 않은 개념이었다. PC 모니터는 거의 비슷한 색감을 가지고 있었고 그래픽 카드 역시
표현 가능한 색의 범위를 RGB 0 ~ 255 수준으로 관리하면 되었다.
이런 색상 공간을 주로 사용하던 것은 ICC 프로필을 사용하는 출판, 사진 업계와 운영체제에서는 애플 진영만이 다양한 색상 공간으로
원본 이미지와 디지털 이미지의 차이를 줄이려고 프로그램을 제작했었다.

하지만 4K, 8K 등의 디스플레이가 등장하고 HDR 등의 고급 색표현 방식이 등장하면서 PC에서도 점점 색상 공간이 다양화 되기 시작했고,
크로스플랫폼이 되면서 스마트폰, 콘솔, PC등 다양한 디스플레이를 사용하게 되면서 더 다양한 색상 공간을 다루게 되었다.

디지털 이미지에서 사용되는 색 공간은 위와 같은 RGB 공간이 대부분이다.
RGB 공간 중 자주 사용하는 것들은 다음과 같다.
*/

/* SRGB */

/*
1996년 정의된 색상 공간으로 마이크로소프트와 HP에서 지정했다. 가장 많이 사용되던 색 공간이다.

이런 형태를 색재현율이라 한다.

3차원 공간의 도형을 평면을 펼친 이미지로 가운데 삼각형이 SRGB가 표현할 수 있는 영역이다.
과거에 정의한 표준이라 매우 적은 색상만 표현할 수 있다. 주위에는 SRGB를 보완하기 위해 등장한 다양한 색상 공간들이다.
*/

/* REC. 709 */

/*
HD TV에서 사용하는 색상 공간이다. 정확히는 해상도, 프레임 등의 정보를 모두 규격한 멀티미디어 규약이다.
RGB 는 각각 8비트 또는 10비트를 사용하여 표현한다.
*/

/* REC. 2020 */

/*
UHD TV의 표준이며, HDR 을 지원하는 색상 공간 중 하나이다.

RGB는 각각 10 비트 또는 12 비트를 사용하여 표현하며, 현 시대 고화질 TV 표현 중 가장 많은 영역의 색상을 표현할 수 있다.
*/

/* DXGI_FORMAT */

/*
이렇게 다양한 색공간이 존재하므로 우리가 알고 있는 RGB 값을 표현하는 규격(포멧)이 필요한데,
이러한 정보를 담고 있는 것이 DXGI_FORMAT 열거형이다. 몇 가지 기본 형태만 살펴 보면 다음과 같다.

_FLOAT : 32 비트 부동소수점 형태로 표현한다.
_SRGB : Standard RGB 데이터로 0 ~ 1 사이의 값을 해석할 때 비선형 방식으로 해석한다.
_UNORM : Unsigned Nomalized 의 약자로 0 ~ 1 사의 값을 고른 산격의 부동 소수로 해석한다.
*/