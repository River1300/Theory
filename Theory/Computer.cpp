...
// 컴퓨터의 모든 계산을 담당하는 것이 중앙처리장치( CPU : Central Processing Unit )이다.
// 문제는 각 CPU는 자체 연산을 최적화하기 위해 내부에서는 표준 규격 없이 자체적인 기술들을 사용한다는 것이다.
// 이로 인해 낮은 수준 값에 접근을 하게 되면 CPU 마다 다른 결과를 볼 수 있다.

/*
대표적인 예가 바이트 순서( Byte Order )라는 것이다.

정수는 메모리에 4바이트를 차지한다.
정수 1은 16진수 4바이트로 표현하면 0x0000001 이다.

CPU 제조 업체마다 바이트 단위로 메모리에 저장하는 방식이 다르다.
Big-Endian
	방식 : MSB( Most Significant Byte )데이터를 순서대로 기록
	장점 : 데이터 구조를 사람이 읽기 쉬워 디버깅이 편리함
	사용 예 : PowerPC, 모토로라, Sparc->리눅스 서버 및 네트워크
	0x1 :			00 00 00 01
	0x12 34 :		00 00 12 34
	0x12 34 56 78 : 12 34 56 78
Little-Endian
	방식 : LSB( Least Significant Byte )
	장점 : 산술 연산이 빠른
	사용 예 : Intel(x86, x64)-> 윈도우
	0x1 :			01 00 00 00
	0x12 34 :		34 12 00 00
	0x12 34 56 78 : 78 56 34 12
*/

// 윈도우, 최신 맥은 모두 Little-Endian 이다. 그런데 Java는 Big-Endian을 사용한다.
// 즉 프로그래밍 언어에서 기본으로 사용하는 바이트 순서도 다를 수 있으므로 주의해야 한다.
// 무엇보다 네트워크로 데이터를 주고 받을 때는 서버와 클라이언트에서 엔디언을 맞춰야 하므로 개념 정도는 알두자.